...
static void pend_req_pin_idle(struct lpuart_data *data)
{
	...
	while (nrf_gpio_pin_read(data->req_pin) == 0) {
	}
...
static void req_pin_set(struct lpuart_data *data)
{
	...
	err = nrfx_gpiote_pin_int_config(data->req_pin, NRFX_GPIOTE_INT_DISABLE,
					 NULL, NULL);
	__ASSERT_NO_MSG(err == NRFX_SUCCESS);

	if (0) {
		nrf_gpio_pin_set(data->req_pin);
		nrf_gpio_cfg(data->req_pin,
			     NRF_GPIO_PIN_DIR_OUTPUT,
			     NRF_GPIO_PIN_INPUT_DISCONNECT,
			     NRF_GPIO_PIN_PULLUP,
...
static void req_pin_arm(struct lpuart_data *data)
{
	...
	err = nrfx_gpiote_pin_int_config(data->req_pin, NRFX_GPIOTE_INT_ENABLE,
					 NULL, NULL);
	__ASSERT_NO_MSG(err == NRFX_SUCCESS);
}
...
static int req_pin_init(struct lpuart_data *data, nrfx_gpiote_pin_t pin)
{
	...
	int_flags = NRFX_GPIOTE_INT_CFG_PRESENT | NRFX_GPIOTE_INT_LOW |
		    NRFX_GPIOTE_INT_EDGE | NRFX_GPIOTE_INT_HANDLER |
		    NRFX_GPIOTE_INT_CHAN(ch);

	err = nrfx_gpiote_pin_config(pin, NRFX_GPIOTE_INPUT);
	__ASSERT_NO_MSG(err == NRFX_SUCCESS);
	...
	err = nrfx_gpiote_pin_int_config(pin, int_flags, req_pin_handler, data);
	__ASSERT_NO_MSG(err == NRFX_SUCCESS);
...
static void rdy_pin_suspend(struct lpuart_data *data)
{
	...
	err = nrfx_gpiote_pin_int_config(data->rdy_pin, NRFX_GPIOTE_INT_DISABLE,
					 NULL, NULL);
	__ASSERT_NO_MSG(err == NRFX_SUCCESS);
}
...
{
	uint32_t int_flags = NRFX_GPIOTE_INT_HANDLER;
	nrfx_err_t err;
	...
	err = nrfx_gpiote_pin_int_config(pin, int_flags, rdy_pin_handler, data);
	__ASSERT_NO_MSG(err == NRFX_SUCCESS);
...
/* Pin activated to detect high state. */
static void rdy_pin_idle(struct lpuart_data *data)
{
	nrfx_err_t err;
	uint32_t int_flags = NRFX_GPIOTE_INT_DISABLE;

	err = nrfx_gpiote_pin_int_config(data->rdy_pin, int_flags, NULL, NULL);
	__ASSERT_NO_MSG(err == NRFX_SUCCESS);

	int_flags = NRFX_GPIOTE_INT_CFG_PRESENT |
		    NRFX_GPIOTE_INT_HIGH |
		    NRFX_GPIOTE_INT_ENABLE;
...
{
	nrfx_err_t err;
	uint32_t int_flags = NRFX_GPIOTE_INT_CFG_PRESENT |
			     NRFX_GPIOTE_INT_LOW | NRFX_GPIOTE_INT_EDGE |
			     NRFX_GPIOTE_INT_CHAN(data->rdy_ch) |
			     NRFX_GPIOTE_INT_ENABLE;

	err = nrfx_gpiote_pin_int_config(data->rdy_pin, NRFX_GPIOTE_INT_DISABLE, NULL, NULL);
